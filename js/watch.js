...
// Simple, consolidated button handlers for pause/stop seeding - added at the end to ensure they get attached
document.addEventListener('DOMContentLoaded', function() {
    console.log('Setting up consolidated button handlers');
    
    // Get the buttons
    const pauseButton = document.getElementById('btn-continue-seeding');
    const stopButton = document.getElementById('btn-stop-seeding');
    const seedOnlyButton = document.querySelector('.btn-download');
    
    // PAUSE BUTTON - Direct, simple implementation
    if (pauseButton) {
        // Remove any existing handlers to avoid duplication
        pauseButton.replaceWith(pauseButton.cloneNode(true));
        // Get the fresh reference
        const freshPauseButton = document.getElementById('btn-continue-seeding');
        
        freshPauseButton.addEventListener('click', function() {
            ...
            if (window.currentTorrent) {
                ...
                if (window.isSeedingPaused) {
                    ...
                    freshPauseButton.innerHTML = '<i class="fas fa-play"></i> Resume Seeding';
                    freshPauseButton.classList.add('paused');
                } else {
                    // Resume the torrent
                    window.currentTorrent.resume();
                    console.log('Torrent resumed');
                    freshPauseButton.innerHTML = '<i class="fas fa-pause"></i> Pause Seeding';
                    freshPauseButton.classList.remove('paused');
                }
    ...
    // STOP BUTTON - Direct, simple implementation
    if (stopButton) {
        // Remove any existing handlers to avoid duplication
        stopButton.replaceWith(stopButton.cloneNode(true));
        // Get the fresh reference
        const freshStopButton = document.getElementById('btn-stop-seeding');
        
        freshStopButton.addEventListener('click', function() {
            console.log('Stop button clicked');
            stopSeedingAndClaimPoints();
        });
    }
    
    // SEED ONLY BUTTON - Direct, simple implementation
    if (seedOnlyButton) {
        // Remove any existing handlers to avoid duplication
        const freshSeedOnlyButton = seedOnlyButton.cloneNode(true);
        seedOnlyButton.parentNode.replaceChild(freshSeedOnlyButton, seedOnlyButton);
        
        freshSeedOnlyButton.addEventListener('click', function() {
            console.log('Seed Only button clicked');
            if (!freshSeedOnlyButton.classList.contains('seeding-active')) {
                freshSeedOnlyButton.classList.add('seeding-active');
                freshSeedOnlyButton.innerHTML = '<i class="fas fa-compact-disc fa-spin"></i> Seeding...';
                
                const torrentId = document.querySelector('meta[name="torrent-id"]').getAttribute('content');
                console.log('Starting seeding for torrent:', torrentId);
                
                // Ensure we have a WebTorrent client
                if (!window.client) {
                    window.client = new WebTorrent();
                    console.log('Created new WebTorrent client for Seed Only');
                }
                
                // Add the torrent and setup handlers
                window.client.add(torrentId, function(torrent) {
                    window.currentTorrent = torrent;
                    console.log('Torrent added for seeding only');
                    
                    // Enable controls now that we're seeding
                    const pauseButton = document.getElementById('btn-continue-seeding');
                    const stopButton = document.getElementById('btn-stop-seeding');
                    ...
                    // Setup seeding state and UI
                    window.isReportingMetrics = true;
                    window.isSeedingPaused = false;
                    window.seedingStartTime = new Date();
                    
                    // Start reporting metrics
                    startReportingMetrics();
                    
                    // Show the torrent info section
                    const torrentInfo = document.getElementById('torrent-info');
                    if (torrentInfo) {
                        torrentInfo.style.display = 'grid';
                    }
    ...
    console.log('Consolidated button handlers set up');
});
